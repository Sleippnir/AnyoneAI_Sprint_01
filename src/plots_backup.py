...
#     overall_min = min(y1_min, y2_min)
#     overall_max = max(y1_max, y2_max)
    
#     # Set both axes to use the same scale
#     ax1.set_ylim(overall_min, overall_max)
#     ax2.set_ylim(overall_min, overall_max)
    
#     # Labels and title
#     ax1.set_xlabel("Month")
#     ax1.set_ylabel("Real Delivery Time (days)")
#     ax2.set_ylabel("Estimated Delivery Time (days)")
#     ax1.set_title(f"Average Days Delivery Time by Month in {year}")
    
#     # Legend
#     lines = line1.get_lines() + line2.get_lines()
#     labels = ["Real time", "Estimated time"]
...
def plot_freight_value_weight_relationship(df: DataFrame):
    ...
    plt.title('Freight Value vs Product Weight Relationship')
...
def plot_order_amount_per_day_with_holidays(df: DataFrame):
    ...
    for _, row in holiday_rows.iterrows():
        plt.axvline(x=row['date'], color='red', linestyle='--', 
            alpha=0.7, label='Holiday' if _ == 0 else "")
    
    # Formatting
    plt.title('Daily Order Count with Holiday Indicators')
    plt.xlabel('Date')
