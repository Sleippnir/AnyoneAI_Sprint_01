...
def query_freight_value_weight_relationship(database: Engine) -> QueryResult:
    ...
    data = pd.merge(merged, products, on='product_id', how='inner')
    ...
    }).reset_index()  # Reset index to keep order_id as a column
...
def query_orders_per_day_and_holidays_2017(database: Engine) -> QueryResult:
    ...
    # Rename "order_purchase_timestamp" column to 'date'
    ...
    # Convert the 'date' column in holidays DataFrame to a set of dates for quick lookup
    holiday_dates = set(pd.to_datetime(holidays["date"]).dt.date
    )
    
    # Create the result DataFrame with the order count and date
    # and add boolean column if the date is a holiday
    result_df = order_purchase_ammount_per_date.assign(
        holiday = order_purchase_ammount_per_date["date"].isin(holiday_dates)
    )
